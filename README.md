...
It predicts the next token—but in doing so, it can build surprisingly rich internal representations of the information it has seen. but without understanding any single word .I think there is an important distinction between being highly trained and being intelligent.
An AI model is trained extremely well to predict the next token based on the patterns it has learned from enormous amounts of data. Within the space of those learned patterns, it can be remarkably good at predicting what comes next, extracting certain relationships, recognizing patterns, and even performing some forms of reasoning.
But I don't think that, by itself, should be called intelligence. It is evidence of extremely sophisticated training.
For example, you can train an AI model to recognize and extract a particular type of relationship. You can train it on patterns such as:
«A is greater than B, and B is greater than C, therefore A is greater than C.»
The model may become extremely good at recognizing and applying this particular structure. But that does not necessarily mean that it has a general ability to discover any kind of relationship between any two things when faced with a completely new situation.
This is similar to a lion trained in a circus. The lion can be trained extremely well to perform specific movements. It can jump, stand, roll over, or perform a particular sequence of actions with impressive accuracy. But the fact that it performs those actions very well does not necessarily mean that it has developed a general intelligence that allows it to figure out completely new actions on its own.
If you ask the lion to perform a completely new movement, it would generally need to be trained for that movement first. And even if it manages to perform it without specific training, it is unlikely to perform it with the same consistency and precision as the actions it was specifically trained to perform.
I see current AI in a similar way. It is extremely well trained to predict the next token within the patterns and structures it has learned. That can produce remarkably sophisticated behavior, but sophisticated behavior resulting from training is not necessarily the same thing as intelligence.
For me, intelligence involves the ability to encounter genuinely new information, learn from it, adapt, update what you know, and use that new knowledge to deal with future situations.
A trained system can be extremely good at what it was trained to do. But intelligence, in the stronger sense, is the ability to go beyond the training itself.
An AI can be extraordinarily well trained without necessarily being intelligent. Predicting the next token extremely well is a remarkable training achievement, but it is not, by itself, proof of intelligence. so I don't think this is intelligent at any form of intelligent
